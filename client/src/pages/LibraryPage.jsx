...
import { Search, Clapperboard, X, ListVideo } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">Search, filter, and manage everything you&apos;ve generated.</p>
      </div>

      {activeProject && (
        <div className="flex items-center gap-2 text-sm">
          <span className="text-muted-foreground">Filtered by project:</span>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 text-primary border border-primary/20 px-2.5 py-0.5 text-xs font-medium">
            {activeProject.name}
            <button onClick={clearProjectFilter} aria-label="Clear project filter" className="hover:text-primary/70">
              <X className="h-3 w-3" />
            </button>
          </span>
        </div>
