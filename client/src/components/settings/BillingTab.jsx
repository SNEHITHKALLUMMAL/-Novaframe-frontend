...
export function BillingTab() {
  ...
  // Returning from Stripe Checkout — PAYMENT_SUCCESS_URL/PAYMENT_CANCEL_URL
  // (see server .env.example) land back here with a ?checkout= query param.
  // The subscription itself is only actually updated once the webhook has
  // been processed, which can trail slightly behind this redirect landing
  // — refetching covers the common case; a manual refresh covers the rest.
  useEffect(() => {
    ...
    if (checkout === 'success') {
      toast({ title: 'Payment received', description: "Your plan will update shortly.", variant: 'success' });
      invalidateAll();
  ...
  const subscribeMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.checkoutUrl) {
        // Paid plan — hand off to Stripe's hosted checkout page. The
        // subscription itself isn't updated yet; it becomes active once
        // the webhook confirms payment and the user lands back here via
        // PAYMENT_SUCCESS_URL.
